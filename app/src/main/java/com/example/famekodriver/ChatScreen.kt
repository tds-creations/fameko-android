            ...
            TopAppBar(
                ...
                title = { 
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Column {
                            Text(customerName, fontSize = 16.sp, fontWeight = FontWeight.Bold, color = BoltDark)
                            Text("Active Trip • Customer", fontSize = 11.sp, color = FamekoSecondary, fontWeight = FontWeight.Medium)
                        }
                    }
                },
                navigationIcon = {
        ...
        Box(modifier = Modifier.padding(padding)) {
            if (isLoading && messages.isEmpty()) {
                ...
                Column(
                    ...
                    Text("Communicate with your customer here", fontSize = 14.sp, color = Color.Gray, textAlign = TextAlign.Center)
                }
            } else {
                LazyColumn(
                    state = listState,
...
@Composable
fun DriverMessageBubble(message: Message, isMe: Boolean, time: String) {
    Column(
        modifier = Modifier.fillMaxWidth().padding(vertical = 4.dp),
        horizontalAlignment = if (isMe) Alignment.End else Alignment.Start
    ) {
        Surface(
            color = if (isMe) FamekoSecondary else BoltLightGray,
