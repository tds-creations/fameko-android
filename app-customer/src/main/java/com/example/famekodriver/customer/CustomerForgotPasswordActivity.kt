            ...
            if (!isOtpSent) {
                ...
                lifecycleScope.launch {
                    repository.forgotPassword(email).onSuccess { resp ->
                        if (resp.success) {
                            ...
                            Toast.makeText(this@CustomerForgotPasswordActivity, "Check your email for the code", Toast.LENGTH_LONG).show()
                        } else {
                            btnAction.isEnabled = true
                            Toast.makeText(this@CustomerForgotPasswordActivity, resp.message ?: "Error", Toast.LENGTH_SHORT).show()
                        }
                    }.onFailure {
                        btnAction.isEnabled = true
                        Toast.makeText(this@CustomerForgotPasswordActivity, "Failed to send code", Toast.LENGTH_SHORT).show()
                    }
                }
            } else {
                ...
                lifecycleScope.launch {
                    repository.resetPassword(email, otp, newPass).onSuccess { resp ->
                        if (resp.success) {
                            Toast.makeText(this@CustomerForgotPasswordActivity, "Password reset successful! Please login.", Toast.LENGTH_LONG).show()
                            finish()
                        } else {
                            btnAction.isEnabled = true
                            Toast.makeText(this@CustomerForgotPasswordActivity, resp.message ?: "Invalid code", Toast.LENGTH_SHORT).show()
                        }
                    }.onFailure {
                        btnAction.isEnabled = true
                        Toast.makeText(this@CustomerForgotPasswordActivity, "Reset failed", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        }
